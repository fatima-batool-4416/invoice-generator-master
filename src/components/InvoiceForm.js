...
class InvoiceForm extends React.Component {
  ...
  handleAddEvent = () => {
    const newItem = {
      id: new Date().getDate() + + Math.floor(Math.random() * 999999),
      name: '',
      price: '1.00',
      description: '',
      quantity: '1'
  ...
  handleCalculateTotal = () => {
    ...
    const total = (subTotal - discountAmmount + taxAmmount).toFixed(2);
...
onItemizedItemEdit = (event) => {
  ...
  const updateItem = this.state.items.map(item =>
    ...
    this.setState({ item: updateItem }, this.handleCalculateTotal);
};
